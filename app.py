...
def show_invoice():
    ...
    c.close
    conn.close()
    return(render_template("scan_invoice/show_invoice.html", username=username, r=r))
...
def fin_result():
    ...
    # Split lines by newline characters
        ...
        formatted_lines = []
        for line in lines:
            # Add a newline before lines starting with '*' (bullet points)
            if line.startswith("*"):
    ...
    prompt2 += "In a new paragraph, list credits card in Singapore that are suitable for me. "
    
    r = palm.chat(**model, messages=prompt)
    r2 = palm.chat(**model, messages=prompt2)
    fin_res = format_response(r.last)
    fin_res2 = format_response(r2.last) #Markup(format_response(r2.last))

    #############################################################
    ## plot chart but not sure why is it not working ############
    ## theres some static upload folder logic; can't understand #
    #############################################################
    def plot_bars(income, expense, asset, debt):
        ...
        # Asset and Debt chart
        labels = ["Asset", "Debt"]
        values = [asset, debt]
        colors = ["green", "red"]
        ax2.bar(labels, values, color=colors)
        ax2.set_title("Asset vs. Debt")
        ...
        # Save the figure as a temporary PNG image
        fig.savefig("financial_charts.png", bbox_inches="tight")
        plt.close(fig)
        return "financial_charts.png"  # Return the image filename

    image_filename = plot_bars(income, expense, asset, debt)

    return(render_template("fin_health/fin_result.html", username=username, fin_res=fin_res, fin_res2 = fin_res2, image_filename = image_filename))
...
def delete_account():
    ...
    c.close
    conn.close()
    return(render_template("delete_account/delete_account.html", username=username, r=r))

@app.route("/delete_acc_cfm",methods=["GET","POST"])
def delete_acc_cfm():
    global username
    username = request.form.get('username')
    password = request.form.get('password')
    ...
    c.close
    conn.close()
    return(render_template("delete_account/delete_acc_cfm.html", username=username, r=r))
...
def account_delete():
    ...
    c.close
    conn.close()
    return(render_template("log/account_delete.html"))
